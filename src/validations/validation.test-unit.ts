...
const rq = (headers: Headers = new Headers({ Accept: 'application/json' })): Request =>
  <any>{
    headers,
  };
...
describe('validateResponse', () => {
  ...
  test('throws if the response status is not within the acceptable range', () => {
    expect(() => validateResponse(rq(), rs(300), buildOptions())).toThrowError(
      ERRORS.UNEXPECTED_RESPONSE_STATUS_CODE,
    );
  });

  test("throws if the response doesn't have the Content-Type Header", () => {
    expect(() => validateResponse(rq(), rs(200, 'OK', new Headers()), buildOptions())).toThrowError(
  ...
  test("a response can be successful even if the Content-Type Header is not identical to the request's Accept Header", () => {
    expect(
      validateResponse(
        rq(),
        rs(200, 'OK', new Headers({ 'Content-Type': 'application/json; charset=utf-8' })),
        buildOptions(),
      ),
    ).toBeUndefined();
  });
  ...
  test("throws if the request' content type is different to the response's", () => {
    expect(() =>
      validateResponse(
        rq(),
        rs(200, 'OK', new Headers({ 'Content-Type': 'text/html' })),
        buildOptions(),
      ),
